...
function createProcesses(
  ...
  return errors.length > 0 ? [[], errors] : [processes, null];
}
...
function createProcess(processData: IUserProcessData, typesByName: Map<string, DataType>, errors: string[]) {
  ...
  return new UserProcess(
    ...
    inputs,
    outputs,
    processData.returnPaths === undefined ? [] : processData.returnPaths.slice(),
    variables,
  );
}
...
function loadParameters(
  ...
  const paramsWithTypes: Array<[IParameterData, DataType | null]> = parameters.map(p => {
    ...
    return [p, type === undefined ? null : type];
  });

  const errorParams = paramsWithTypes.filter(p => p[1] === null).map(p => p[0]);

  for (const param of errorParams) {
    errors.push(`Unrecognised type \"${param.type}\" used by ${paramType} of process ${process.name}`);
  }

  return paramsWithTypes.filter(p => p[1] !== null).map(p => new Parameter(p[0].name, p[1]!));
}

function loadVariables(process: IUserProcessData, typesByName: Map<string, DataType>, errors: string[]) {
  ...
  for (const variableData of process.variables) {
    ...
    if (dataType === undefined) {
      errors.push(
        `Unrecognised type \"${variableData.type}\" used by variable ${variableData.name} of process ${process.name}`,
      );
      continue;
    }

    const value =
      variableData.initialValue !== undefined && isDeserializable(dataType)
...
): [Map<string, Process>, string[] | null] {
  const errors: string[] = [];

  if (!areNamesUnique(userProcesses.map(p => p.name), errors)) {
    return [new Map<string, Process>(), errors];
  }
...
function loadProcessSteps(
  ...
  for (const stepData of steps) {
    ...
    if (isStartStep(stepData)) {
      ...
      errors.push(
        `Invalid type "${(stepData as any).type}" on step ${(stepData as any).id} in process "${process.name}"`,
      );
    }
  }
  ...
  for (const [stepData, step, parameters] of stepsWithOutputs) {
    ...
    if (stepData.returnPath !== undefined) {
      ...
      if (destStep !== undefined) {
        ...
        errors.push(
          `Step ${step.id} in process "${process.name}" tries to connect to non-existent step "${stepData.returnPath}"`,
        );
      }
      ...
      for (const returnPath of Object.keys(stepData.returnPaths)) {
        if (expectedReturnPaths.indexOf(returnPath) === -1) {
          errors.push(
            `Step ${step.id} in process "${process.name}" tries to map unexpected return path "${returnPath}"`,
          );
          continue;
        }
        ...
        if (destStep === undefined) {
          errors.push(
            `Step ${step.id} tries to connect to non-existent step "${destName}" in process "${process.name}"`,
          );
          continue;
        }
        ...
        if (mappedPaths.has(returnPath)) {
          errors.push(
            `Step ${step.id} in process "${process.name}" tries to map the "${returnPath}\" return path multiple times`,
          );
          continue;
        }
...
function mapParameters(
  ...
  for (const paramName of Object.keys(paramData)) {
    ...
    if (variable === undefined) {
      const paramType = isInputParam ? 'input' : 'output';
      errors.push(
        `Step ${step.id} tries to map an ${paramType} to non-existent variable "${variableName}" in process "${process.name}"`,
      );
      continue;
    }
...
function checkUnassignedVariables(process: UserProcess, errors: string[]) {
  const unassignedVariables = new Set<Variable>(process.variables.filter(v => v.initialValue === null));
  ...
  for (const nextStep of nextSteps) {
    ...
    for (const variable of nextStep.inputMapping.values()) {
      if (unassignedVariables.has(variable)) {
        errors.push(
          `Step ${currentStep.id} in process "${process.name}" uses a variable before it is assigned: ${variable.name}`,
        );
    ...
    if (
      isUserStep(nextStep) &&
      !checkUnassignedVariablesRecursive(process, nextStep, visitedSteps, unassignedVariables, errors)
    ) {
      allValid = false;
    }
  }
